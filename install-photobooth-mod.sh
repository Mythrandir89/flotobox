...
start_install() {
    ...
    if [ $GIT_INSTALL = true ]; then
        git clone https://github.com/Mythrandir89/flotobox $INSTALLFOLDER
        cd $INSTALLFOLDERPATH
        #add_git_remote
        start_git_install
    else
...
gphoto_preview() {
    if [ -d "/etc/systemd/system" ] && [ -d "/usr" ]; then
        wget https://raw.githubusercontent.com/Mythrandir89/flotobox/Original-Source/gphoto/ffmpeg-webcam.service -O "/etc/systemd/system/ffmpeg-webcam.service"
        wget https://raw.githubusercontent.com/Mythrandir89/flotobox/Original-Source/gphoto/ffmpeg-webcam.sh -O "/usr/ffmpeg-webcam.sh"
